...
import { dispatchIikoOutbox } from '~/server/utils/iiko'
...
export default defineEventHandler(async (event) => {
  ...
  const result = await dispatchIikoOutbox(db, access.shopId, (shop as any)?.integration_keys ?? {})
  await db.from('iiko_sync_jobs').insert({
