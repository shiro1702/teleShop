...
export default defineEventHandler(async (event) => {
  ...
  if (access.role !== 'owner') throw createError({ statusCode: 403, statusMessage: 'Only owner can run health-check' })
  ...
  const { client, config } = getIikoClient((shop as any)?.integration_keys ?? {})
  if (config.mode === 'http' && !config.apiKey) throw createError({ statusCode: 400, statusMessage: 'apiKey is required for http mode' })
