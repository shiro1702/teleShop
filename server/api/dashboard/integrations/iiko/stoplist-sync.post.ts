...
export default defineEventHandler(async (event) => {
  ...
  const { client } = getIikoClient((shop as any)?.integration_keys ?? {})

  let mappingQuery = db.from('iiko_restaurant_mapping').select('restaurant_id,iiko_terminal_group_id').eq('shop_id', access.shopId)
  if (body.restaurantId) mappingQuery = mappingQuery.eq('restaurant_id', body.restaurantId)
  const { data: mappings } = await mappingQuery
  if (!mappings?.length) throw createError({ statusCode: 400, statusMessage: 'No iiko restaurant mappings configured' })

  const updated: Array<{ restaurantId: string; affected: number }> = []
  for (const map of mappings) {
    const stopList = await client.fetchStopList((map as any).iiko_terminal_group_id)
    let affected = 0
